...
@CheckData(
        name = "AnalysisC",
        configName = "AnalysisC",
        type = CheckType.ANALYSIS,
        decay = 0.85,
        experimental = true
)
public class AnalysisC extends Check implements RotationCheck {
    ...
    public AnalysisC(PlayerData player) {
        super(player);
    }
    ...
    public void process(RotationUpdate rotationUpdate) {
        if (!player.hasAttackedSince(1400L)) {
            rotation.clear();
            return;
        }
        ...
        if (player.getTarget().getType() != EntityTypes.PLAYER) return;
    ...
    public void checkForAimbot(RotateProcessor processor, RotationUpdate update) {
        if (rotations == null) {
            return;
        }
    ...
    private void checkAdvancedPatterns(List<Float> yaws, float avgYaw) {
        ...
        if (absKurtosis > kurtosisThreshold) {
            ...
            if (similarCount > 2 && avgYaw > 5) { 
                if (time() - lastFlag4 < 500L) {
                    return;
                }
