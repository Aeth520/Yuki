...
public final class BoundingBoxSize {
    ...
    private static float getWidthMinusBaby(PlayerData player, PacketEntity packetEntity) {
        ...
        if (EntityTypes.AXOLOTL.equals(type)) {
            ...
            return player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_9) ? 0.4f : 0.6f;
                ...
                || EntityTypes.STRIDER.equals(type)
                || EntityTypes.COW.equals(type)
                || EntityTypes.SHEEP.equals(type)
                || EntityTypes.MOOSHROOM.equals(type)
                || EntityTypes.PIG.equals(type)
                || EntityTypes.LLAMA.equals(type)
                || EntityTypes.DOLPHIN.equals(type)
                || EntityTypes.WITHER.equals(type)
                || EntityTypes.TRADER_LLAMA.equals(type)
                || EntityTypes.WARDEN.equals(type)
                || EntityTypes.GOAT.equals(type)) {
            return 0.95f; 
            ...
            return 1.5f;
        } else if (packetEntity instanceof PacketEntityGuardian packetEntityGuardian) { 
    ...
    public static Vector3d getRidingOffsetFromVehicle(PacketEntity entity, PlayerData player) {
        ...
        if (entity instanceof PacketEntityTrackYaw entityYaw) {
            ...
            if (EntityTypes.isTypeInstanceOf(entity.getType(), EntityTypes.BOAT)) {
                float f = 0f;
                float f1 = (float) (getPassengerRidingOffset(player, entity) - 0.35f); 
                ...
                vec3 = yRot(MathUtil.radians(-entityYaw.interpYaw) - ((float) Math.PI / 2f), vec3);
                return new Vector3d(x + vec3.x, y + (double) f1, z + vec3.z);
            } else if (entity.getType() == EntityTypes.LLAMA) {
                float f = player.trigHandler.cos(MathUtil.radians(entityYaw.interpYaw));
                float f1 = player.trigHandler.sin(MathUtil.radians(entityYaw.interpYaw));
                ...
                float f = player.trigHandler.sin(MathUtil.radians(entityYaw.interpYaw));
                float f1 = player.trigHandler.cos(MathUtil.radians(entityYaw.interpYaw));
