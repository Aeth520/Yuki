...
import cn.aetheris.yuki.protocol.nms.vec.Vec2i;
...
@CheckData(name = "AnalysisE", type = CheckType.ANALYSIS, configName = "AnalysisE", decay = 0.85, experimental = true)
public final class AnalysisE extends Check implements RotationCheck {
    ...
    public AnalysisE(PlayerData player) {
        super(player);
        this.rawRotations = new CopyOnWriteArrayList<>();
        this.rotations2 = new CopyOnWriteArrayList<>();
    ...
    private void checkRaw(RotationUpdate update) {
        ...
        if (validSensitivity && deltaYaw < 0.00015 && deltaPitch < 0.00015) {
            ...
            if (bufferStability > 25.0) {
                if (alert("(Stability)\nsy= " + String.format("%.3f", shannonYaw) + "\npd= " + String.format("%.5f", Math.abs(shannonPitch - oldShannonPitch)) + "\nyd= " + String.format("%.5f", Math.abs(shannonYaw - oldShannonYaw)))) {
                    decayBuffer(bufferStability, 0.84, 0.25);
        ...
        if (deltaYaw < 0.005 && shannonPitch < 0.1 && player.isOnGround()) {
        ...
        if (!invalidSensitivity) {
            ...
            if (shannonYaw > 3.1 && shannonPitch < 2.0 && player.getPitch() < 37.5 && avgYaw > 0.085) {
                ...
                if (bufferLowCombo > 5.0) {
                    if (flagAndAlert(String.format("(LowCombo)\nsy= %.3f\nsp= %.3f", shannonYaw, shannonPitch) + "\nv= " + validSensitivity)) {
                        player.mitigateDamage();
                        decayBuffer(bufferLowCombo, 0.85, 0.25);
                    }
                } else {
                    decayBuffer(bufferLowCombo, 1.0, 0.15);
                }
            }
            ...
            if ((entropyRatio > 1.7 || entropyRatio < 0.45) && !isLowPitchSpike) {
                ...
                if (bufferRatio > 3.0) {
                    ...
                    decayBuffer(bufferRatio, 0.95, 0.15);
                }
            }
    ...
    private void checkSpikes() {
        ...
        if (kireikoGeneric.size() >= 7) {
            ...
            if (stdDevX > 5 && stdDevX < 22 && spikeRange.getY() < 50) {
                ...
                if (bufferSpikesRule > 7.0) {
                    if (alert("(Spike Rule) \nstdDevX=" + (int) stdDevX)) {
                        player.mitigateDamage();
                        Bukkit.getScheduler().runTaskLaterAsynchronously(Yuki.getInstance(), (Runnable) player::mitigateDamage, 20L);
                    }
                }
            }
        }
        ...
        if (smallSpikeCount >= 3) {
            ...
            if (bufferSpikesDenseSmall > 5.2) {
                if (flagAndAlert("(Dense Small)\ncount= " + smallSpikeCount)) {
                    player.mitigateDamage();
                    decayBuffer(bufferSpikesDenseSmall, 0.75, 0.25);
                }
            } else {
                decayBuffer(bufferSpikesDenseSmall, 0.95, 0.25);
            }
            smallSpikeCount = 0;
        }
        ...
        if (gcdYaw.size() > 8) {
            ...
            if (microEntropy < 0.8 && avgChange > 28 && currentMax < 45 && MathUtil.getMax(last8) - MathUtil.getMin(last8) > 60) {
                ...
                decayBuffer(bufferMicroSpikes, 0.9, 0.2);
            }
        }
            ...
            if (stability < 0.4 && currentMax > 45) {
                ...
                if (bufferSpikesStability > 3.8) {
                    if (flagAndAlert("(Stability)\nstability= " + String.format("%.2f", stability))) {
                        player.mitigateDamage();
                        decayBuffer(bufferSpikesStability, 0.75, 0.25);
                    }
                } else {
                    decayBuffer(bufferSpikesStability, 0.95, 0.25);
                }
            }
        }
        ...
        if (!yawOutY.isEmpty() && yawOutY.size() < 4) {
            if (currentMax > 150 && player.getRotateProcessor().getDeltaYaw() < 25) {
                bufferSpikesMega = modifyBuffer(bufferSpikesMega, 2.0);
                if (bufferSpikesMega > 5.5) {
                    if (alert("(Mega Spike)\nval= " + currentMax)) {
                        player.mitigateDamage();
                        decayBuffer(bufferSpikesMega, 0.75, 0.25);
                    }
                } else {
                    decayBuffer(bufferSpikesMega, 0.95, 0.15);
                }
            }
                ...
                if (lastSpikeTime != 0 && time() - lastSpikeTime < 1200) {
                    ...
                    if (bufferSpikesRapid > 3.5) {
                        if (flagAndAlert("(Rapid Spikes)\ncount= " + yawOutY.size())) {
                            player.mitigateDamage();
                            decayBuffer(bufferSpikesRapid, 0.85, 0.25);
                        }
                    } else {
                        decayBuffer(bufferSpikesRapid, 0.95, 0.15);
                    }
                    lastSpikeTime = time();
                }
                ...
                if (currentMin > 10 && currentMax / currentMin > 3.5) {
                    ...
                    if (bufferSpikesRatio > 4.2) {
                        if (flagAndAlert("(Ratio Spike)\nmax/min= " + (currentMax / currentMin))) {
                            player.mitigateDamage();
                            decayBuffer(bufferSpikesRatio, 0.85, 0.25);
                        }
                    } else {
                        decayBuffer(bufferSpikesRatio, 0.95, 0.15);
                    }
                }
            }
                ...
                if (rollingStd > 45 && MathUtil.getAverage(gcdYaw) < 30) {
                    ...
                    if (bufferSpikesStd > 3.0) {
                        if (flagAndAlert("(Std Surge)\nstd= " + rollingStd)) {
                            player.mitigateDamage();
                            decayBuffer(bufferSpikesStd, 0.85, 0.25);
                        }
                    } else {
                        decayBuffer(bufferSpikesStd, 0.95, 0.15);
                    }
                }
            }

            if (player.getRotateProcessor().getDeltaYaw() < 25) {
                if ((Math.max(Math.abs(MathUtil.getMax(yawOutY)), Math.abs(MathUtil.getMin(yawOutY))) > 55 && Math.min(Math.abs(MathUtil.getMax(yawOutY)), Math.abs(MathUtil.getMin(yawOutY))) == Math.max(Math.abs(MathUtil.getMax(yawOutY)), Math.abs(MathUtil.getMin(yawOutY)))) || (Math.max(Math.abs(MathUtil.getMax(yawOutY)), Math.abs(MathUtil.getMin(yawOutY))) > 60 && (Math.min(Math.abs(MathUtil.getMax(yawOutY)), Math.abs(MathUtil.getMin(yawOutY))) < Math.max(Math.abs(MathUtil.getMax(yawOutY)), Math.abs(MathUtil.getMin(yawOutY))) / 3))) {
                    bufferSpikesOutlier = modifyBuffer(bufferSpikesOutlier, 1.45);
                    if (bufferSpikesOutlier > 4.0) {
                        if (alert("(Spike Rough) \nOutX= " + yawOutX)) {
    ...
    private boolean isInCombat() {
        return player.hasAttackedSince(800L);
    }

    private boolean hasExemptions() {
        return isExempt(ExemptType.TELEPORT, ExemptType.SERVER_SENT_PULLBACK, ExemptType.SERVER_SENT_ROTATE, ExemptType.ELYTRA_FLYING, ExemptType.VEHICLE) || player.packetStateData.horseInteractCausedForcedRotation || player.getTarget().getType() != EntityTypes.PLAYER || (player.getLastTarget() != null && !player.getTarget().getUuid().equals(player.getLastTarget().getUuid()));
    }
