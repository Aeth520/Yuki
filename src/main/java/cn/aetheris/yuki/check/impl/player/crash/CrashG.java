...
@CheckData(name = "CrashG", type = CheckType.CRASH, configName = "CrashG", description = "Invalid Dig/Placement")
public final class CrashG extends Check implements PacketCheck {
    ...
    private boolean isSupportedVersion() {
        return player.getClientVersion().isNewerThanOrEquals(ClientVersion.V_1_19)
                && Yuki.getInstance().getPacketEventsManager().getServerVersion().isNewerThanOrEquals(ServerVersion.V_1_19);
    }

    private void punish(PacketReceiveEvent event) {
        if (flagAndAlert("type= " + event.getPacketType().getName()) && shouldModifyPackets()) {
            kickPlayer();
