...
public class DynamicConnecting {
    ...
    public boolean connectsTo(PlayerData player, ClientVersion v, int currX, int currY, int currZ, BlockFace direction) {
        ...
        if (BlockTags.STAIRS.contains(target)) {
            ...
            if (v.isOlderThan(ClientVersion.V_1_12) || (Yuki.getInstance().getPacketEventsManager().getServerVersion().isOlderThanOrEquals(ServerVersion.V_1_11) && v.isNewerThanOrEquals(ClientVersion.V_1_13)))
                return false;
    ...
    boolean isBlacklisted(StateType m, StateType fence, ClientVersion clientVersion) {
        ...
        if (BlockTags.STAIRS.contains(fence)) return true; 

        return m == StateTypes.ENCHANTING_TABLE || m == StateTypes.FARMLAND || m == StateTypes.HONEY_BLOCK || m == StateTypes.CARVED_PUMPKIN || m == StateTypes.JACK_O_LANTERN || m == StateTypes.PUMPKIN || m == StateTypes.MELON ||
