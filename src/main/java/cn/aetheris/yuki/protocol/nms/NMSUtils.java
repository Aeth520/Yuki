...
public final class NMSUtils {
    private static ItemUsageReset resetItemUsage;
    private static ItemUsageHandGetter itemUsageHandGetter;

    public static void register() {
        resetItemUsage = createItemUsageResetFunction();
        itemUsageHandGetter = createItemUsageHandGetterFunction();
    }
    ...
    public static void resetItemUsage(@Nullable Player player) {
        ...
        if (player != null) {
            final PlayerData data = PluginLoader.INSTANCE.getPlayerDataManager().getPlayer(PacketEvents.getAPI().getPlayerManager().getUser(player));
            if (data == null) {
                return;
            }
    ...
    @SneakyThrows
    public static @Nullable InteractionHand getItemUsageHand(@Nullable Player player) {
        return player == null ? null
                : itemUsageHandGetter.apply(player);
    }

    @SneakyThrows
    private static @NotNull ItemUsageReset createItemUsageResetFunction() {
        ServerVersion version = Yuki.getInstance().getPacketEventsManager().getServerVersion();
        ...
        if (version == ServerVersion.V_1_8_8) {
            Method getHandle = ReflectionUtils.getClass("org.bukkit.craftbukkit.v1_8_R3.entity.CraftHumanEntity").getMethod("getHandle");
        ...
        String nmsPackage = Bukkit.getServer().getClass().getPackageName().split("\\.")[3];
        Method getHandle = ReflectionUtils.getClass("org.bukkit.craftbukkit." + nmsPackage + ".entity.CraftLivingEntity").getMethod("getHandle");
    ...
    @SneakyThrows
    private static @NotNull ItemUsageHandGetter createItemUsageHandGetterFunction() {
        ServerVersion version = Yuki.getInstance().getPacketEventsManager().getServerVersion();
