...
import cn.aetheris.yuki.block.collision.AxisSelect;
...
public class BlockPlace {
    ...
    private boolean canBeReplaced(StateType heldItem, WrappedBlockState state, BlockFace face) {
        ...
        if (state.getType() == StateTypes.SNOW) {
            ...
            if (heldItem == state.getType() && layers < 8) { 
        ...
        if (state.getType() == StateTypes.VINE) {
            ...
            if (Yuki.getInstance().getPacketEventsManager().getServerVersion().isNewerThanOrEquals(ServerVersion.V_1_13) && !state.isUp())
                return true;
            if (state.getNorth() == North.FALSE) return true;
            if (state.getSouth() == South.FALSE) return true;
            if (state.getEast() == East.FALSE) return true;
    ...
    public void setFaceId(int face) {
        this.faceId = face;
        this.face = Yuki.getInstance().getPacketEventsManager().getServerVersion().isNewerThanOrEquals(ServerVersion.V_1_9)
    ...
    public void set(Vector3i position, WrappedBlockState state) {
        ...
        if (Yuki.getInstance().getPacketEventsManager().getServerVersion().isNewerThanOrEquals(ServerVersion.V_1_13)) {
            if (state.getInternalData().containsKey(StateValue.WATERLOGGED)) { 
    ...
    public BlockFace getPlayerFacing() {
        return BY_2D[MathUtil.floor(player.yaw / 90.0D + 0.5D) & 3];
    }

    public void set() {
        if (material == null) {
            LogUtils.console("&cMaterial " + null + " &chas no placed type!");
            return;
        }
        set(material);
    }
