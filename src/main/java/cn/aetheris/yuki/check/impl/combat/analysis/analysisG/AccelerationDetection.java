package cn.aetheris.yuki.check.impl.combat.analysis.analysisG;


import cn.aetheris.yuki.check.util.rotation.DetectionContext;
import cn.aetheris.yuki.check.util.rotation.StatisticalDetectionStrategy;
...
public class AccelerationDetection implements StatisticalDetectionStrategy {
    ...
    public void detect(PlayerData profile, DetectionContext context) {
        ...
        String debug = String.format(
                "%.3f, %.3f, %.3f, %.3f", yawZScore, pitchZScore, yawMean, pitchMean
        );
        ...
        if (pitchZScore < 0.05 && yawZScore < 0.05 && Math.abs(yawMean) > 200 && Math.abs(pitchMean) > 200 && Math.abs(yawMean) < 800 && Math.abs(pitchMean) < 800) {
            buffer++;
            if (buffer >= 5)
                context.flagDetection(this, String.format("Pattern #1\n%s", debug));
        } else {
            buffer -= 0.4;
