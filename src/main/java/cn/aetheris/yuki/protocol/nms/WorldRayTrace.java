...
public class WorldRayTrace {
    ...
    public static HitData traverseBlocks(PlayerData player, Vector3d start, Vector3d end, BiFunction<WrappedBlockState, Vector3i, HitData> predicate) {
        ...
        double endX = MathUtil.lerp(-1.0E-7D, end.x, start.x);
        double endY = MathUtil.lerp(-1.0E-7D, end.y, start.y); 
        double endZ = MathUtil.lerp(-1.0E-7D, end.z, start.z);
        double startX = MathUtil.lerp(-1.0E-7D, start.x, end.x);
        double startY = MathUtil.lerp(-1.0E-7D, start.y, end.y);
        double startZ = MathUtil.lerp(-1.0E-7D, start.z, end.z);
        int floorStartX = MathUtil.floor(startX);
        int floorStartY = MathUtil.floor(startY);
        int floorStartZ = MathUtil.floor(startZ);
        ...
        double d12 = posXInverse * (xSign > 0 ? 1.0D - MathUtil.frac(startX) : MathUtil.frac(startX));
        double d13 = posYInverse * (ySign > 0 ? 1.0D - MathUtil.frac(startY) : MathUtil.frac(startY));
        double d14 = posZInverse * (zSign > 0 ? 1.0D - MathUtil.frac(startZ) : MathUtil.frac(startZ));
