...
public final class PlayerBaseTick {
    ...
    private static void moveTowardsClosestSpaceLegacy(PlayerData player, double x, double z) {
        int floorX = MathUtil.floor(x);
        int floorZ = MathUtil.floor(z);
        int floorY = MathUtil.floor(player.lastY + 0.5);
        ...
        if (suffocates) {
            ...
            double pushUncertainty = Yuki.getInstance().getPacketEventsManager().getServerVersion().isOlderThanOrEquals(ServerVersion.V_1_8_8) ? 0.17 : 0.1;
    ...
    private static boolean updateFluidHeightAndDoFluidPushingLegacy(PlayerData player, FluidTag tag, double multiplier) {
        ...
        int floorX = MathUtil.floor(aABB.minX);
        int ceilX = MathUtil.ceil(aABB.maxX);
        int floorY = MathUtil.floor(aABB.minY);
        int ceilY = MathUtil.ceil(aABB.maxY);
        int floorZ = MathUtil.floor(aABB.minZ);
        int ceilZ = MathUtil.ceil(aABB.maxZ);
