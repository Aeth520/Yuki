...
@CheckData(name = "AimB", type = CheckType.AIM, configName = "AimB", experimental = true, decay = 0.75, setback = 7)
public final class AimB extends Check implements RotationCheck {
    ...
    private double lastSensitivityX, lastSensitivityY;
    ...
    public void process(RotationUpdate rotationUpdate) {
        ...
        if (lastTick == -1 || currentTick != lastTick + 1 || !player.hasAttackedSince(1L)) {
        ...
        if (isExempt(
                ...
                ExemptType.VEHICLE) || player.getTarget().getType() != EntityTypes.PLAYER) {
            buffer *= 0.85;
            return;
        }

        double sensitivityX = processor.getSensitivityX();
        double sensitivityY = processor.getSensitivityY();
        ...
        yawAccelHistory.add((double) processor.getYawAccel());
        pitchAccelHistory.add((double) processor.getPitchAccel());

        if (processor.getDeltaYaw() < 0.4 || processor.getDeltaPitch() < 0.2) {
            lastTick = currentTick;
            return;
        }
