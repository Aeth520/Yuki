package cn.aetheris.yuki.util.materials;
...
public class Materials {
    ...
    static {
        ...
        CLIENT_SIDE.addAll(BlockTags.TRAPDOORS.getStates().stream()
                .filter(type -> type != StateTypes.IRON_TRAPDOOR)
    ...
    public static boolean isTrapdoor(StateType type) {
        return BlockTags.TRAPDOORS.contains(type) || type.getName().contains("DOOR");
    }
    ...
    public static boolean isSand(final StateType material) {
        return material.toString().contains("SAND");
    }

    public static boolean isGravel(final StateType material) {
    ...
    public static boolean isWaterlogged(ClientVersion clientVersion, WrappedBlockState state) {
        ...
        if (Yuki.getInstance().getPacketEventsManager().getServerVersion().isOlderThan(ServerVersion.V_1_13))
            return false;
