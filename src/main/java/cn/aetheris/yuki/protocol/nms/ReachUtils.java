...
public class ReachUtils {
    ...
    public static Vector3dm getLook(PlayerData player, float yaw, float pitch) {
        if (player.getClientVersion().isOlderThanOrEquals(ClientVersion.V_1_12_2)) {
            float f = player.trigHandler.cos(MathUtil.radians(-yaw) - (float) Math.PI);
            float f1 = player.trigHandler.sin(MathUtil.radians(-yaw) - (float) Math.PI);
            float f2 = -player.trigHandler.cos(MathUtil.radians(-pitch));
            float f3 = player.trigHandler.sin(MathUtil.radians(-pitch));
            return new Vector3dm(f1 * f2, f3, f * f2);
        } else {
            float f = MathUtil.radians(pitch);
            float f1 = MathUtil.radians(-yaw);
