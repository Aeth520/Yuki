...
import cn.aetheris.yuki.predictionengine.predictions.rideable.PredictionEngineHappyGhast;
...
public class MovementTickerHappyGhast extends MovementTickerLivingVehicle {
    ...
    public MovementTickerHappyGhast(PlayerData player) {
        ...
        if (player.vehicleData.vehicleForward != 0.0F) {
            float pitch = player.pitch * 2F;
            float calcForward = player.trigHandler.cos(pitch * (float) (Math.PI / 180.0));
            float calcUpAndDown = -player.trigHandler.sin(pitch * (float) (Math.PI / 180.0));
            if (player.vehicleData.vehicleForward < 0.0F) {
                calcForward *= -0.5F;
                calcUpAndDown *= -0.5F;
            }
