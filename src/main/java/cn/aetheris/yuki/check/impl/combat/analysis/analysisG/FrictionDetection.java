    ...
    public void detect(PlayerData profile, DetectionContext context) {
        ...
        if ((Math.abs(currentYawVelocity) > 100 || Math.abs(currentPitchVelocity) > 100)) {
            ...
            if (Math.abs(correlation) < (senv > 80 ? -0.1 : -0.4)) {
                ...
                if (buffer2 > 6) {
                    buffer2 -= 2;
                    context.flagDetection(this, String.format(
                            "patterned #1: %.2f, v2: %.2f, c: %.2f",
                            currentYawVelocity, currentPitchVelocity, correlation
                    ));
                }
            } else {
        ...
        if (yawDeltas.size() == 15 && pitchDeltas.size() == 15) {
            ...
            if (buffer3 >= 20) {
                buffer3 -= 4;
                context.flagDetection(this, String.format(
                        "combined friction dy: %.2f, dx: %.2f, tags=[%s]",
                        deltaY, deltaX, String.join(",", tags)
                ));
            }
        }
