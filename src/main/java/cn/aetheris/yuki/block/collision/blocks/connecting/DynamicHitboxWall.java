...
public final class DynamicHitboxWall extends DynamicConnecting implements HitBoxFactory {
    @Override
    ...
    private int[] getConnections(PlayerData player, ClientVersion version, WrappedBlockState state, int x, int y, int z) {
        ...
        if (isModernServer()) {
            boolean sixteen = Yuki.getInstance().getPacketEventsManager().getServerVersion().isNewerThan(ServerVersion.V_1_16);
    ...
    private boolean isModernServer() {
        return Yuki.getInstance().getPacketEventsManager().getServerVersion().isNewerThan(ServerVersion.V_1_12_2);
    }
    ...
    private CollisionBox getModernHitBox(int north, int south, int west, int east, int up) {
        ...
        addDirectionalBox(box, north, 5, 0.0D, 11, 14, 11);
        addDirectionalBox(box, south, 5, 5, 11, 14, 16);
        addDirectionalBox(box, west, 0, 5, 11, 14, 11);
        addDirectionalBox(box, east, 5, 5, 16, 14, 11);

        return box;
    }

    private void addDirectionalBox(ComplexCollisionBox box, int direction, double x1, double z1, double x2, double y2, double z2) {
        if (direction == 1) {
            box.add(new HexCollisionBox(x1, 0, z1, x2, y2, z2));
        } else if (direction == 2) {
            box.add(new HexCollisionBox(x1, 0, z1, x2, 16, z2));
        }
    }
