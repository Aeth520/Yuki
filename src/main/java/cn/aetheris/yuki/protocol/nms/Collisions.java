...
public final class Collisions {
    ...
    private static final boolean IS_FOURTEEN = Yuki.getInstance().getPacketEventsManager().getServerVersion().isNewerThanOrEquals(ServerVersion.V_1_14);
    ...
    public static void checkInsideBlocks(PlayerData player, Vector3d from, Vector3d to) {
        ...
        for (Vector3i blockPos : Collisions.boxTraverseBlocks(player, from, to, boundingBox)) {
            ...
            if (player.visitedBlocks.add(MathUtil.asLong(blockPos.getX(), blockPos.getY(), blockPos.getZ()))) {
    ...
    public static Iterable<Vector3i> boxTraverseBlocks(PlayerData player, Vector3d start, Vector3d end, SimpleCollisionBox boundingBox) {
        ...
        if (direction.lengthSquared() < (double) MathUtil.square(0.99999F)) {
            ...
            for (Vector3i blockPos : initialBlocks) {
                if (alreadyVisited == null || !alreadyVisited.contains(MathUtil.asLong(blockPos.getX(), blockPos.getY(), blockPos.getZ()))) {
    ...
    public static void addCollisionsAlongTravel(LongSet alreadyVisited, Set<Vector3i> output, Vector3d start, Vector3d end, SimpleCollisionBox boundingBox) {
        Vector3d direction = end.subtract(start);
        int currentX = MathUtil.floor(start.x);
        int currentY = MathUtil.floor(start.y);
        int currentZ = MathUtil.floor(start.z);
        int stepX = MathUtil.sign(direction.x);
        int stepY = MathUtil.sign(direction.y);
        int stepZ = MathUtil.sign(direction.z);
        ...
        double tDeltaX = tMaxX * (stepX > 0 ? 1.0 - MathUtil.frac(start.x) : MathUtil.frac(start.x));
        double tDeltaY = tMaxY * (stepY > 0 ? 1.0 - MathUtil.frac(start.y) : MathUtil.frac(start.y));
        double tDeltaZ = tMaxZ * (stepZ > 0 ? 1.0 - MathUtil.frac(start.z) : MathUtil.frac(start.z));
        ...
        while (tDeltaX <= 1.0 || tDeltaY <= 1.0 || tDeltaZ <= 1.0) {
            ...
            if (collisionPoint.isPresent()) {
                ...
                double clampedX = MathUtil.clamp(collisionVec.x, currentX + 1.0E-5F, currentX + 1.0 - 1.0E-5F);
                double clampedY = MathUtil.clamp(collisionVec.y, currentY + 1.0E-5F, currentY + 1.0 - 1.0E-5F);
                double clampedZ = MathUtil.clamp(collisionVec.z, currentZ + 1.0E-5F, currentZ + 1.0 - 1.0E-5F);
                int endX = MathUtil.floor(clampedX + boundingBox.getXSize());
                int endY = MathUtil.floor(clampedY + boundingBox.getYSize());
                int endZ = MathUtil.floor(clampedZ + boundingBox.getZSize());

                for (int x = currentX; x <= endX; x++) {
                    for (int y = currentY; y <= endY; y++) {
                        for (int z = currentZ; z <= endZ; z++) {
                            if (alreadyVisited == null || alreadyVisited.add(MathUtil.asLong(x, y, z))) {
