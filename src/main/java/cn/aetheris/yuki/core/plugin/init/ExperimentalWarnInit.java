    ...
    public void init() {
        if (!PluginLoader.INSTANCE.getConfigManager().getConfig().getBoolean("function.experimental")) {
            MHDFScheduler.getAsyncScheduler().runTaskLater(Yuki.getInstance(), () -> {
                LogUtils.console("&3Yuki &8» &c检测到您没有开启实验性模式，这可能会少很多有用的检测");
                LogUtils.console("&3Yuki &8» &c这些检测因为还需要长久的测试来保证其的稳定性，所以您不开启就可能会造成一些绕过!");
                LogUtils.console("&3Yuki &8» &c方法 -> settings.yml 内搜索experimental保证其为true即可!");
            }, 20L);
        }
    }
}
